...
## 현재 작업 범위 (매우 중요)
- **백엔드가 구현된 기능은 실제 API를 연동한다.** 현재 연동된 기능: 인증(회원가입/로그인), 객관식 문제 풀이(문제은행 목록·채점 조회·세션 저장), 서술형 문제 풀이(세션 시작·AI 채점/꼬리질문·세션 저장), 오답노트(목록·상세 조회·북마크·삭제), 학습 기록(최근 기록 목록·연속/누적 학습일·일자별 학습량 조회 — `/records`, 홈 배너, 헤더 스트릭 배지).
