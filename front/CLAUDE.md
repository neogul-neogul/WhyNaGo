...
## 현재 작업 범위 (매우 중요)
- **백엔드가 구현된 기능은 실제 API를 연동한다.** 현재 연동된 기능: 인증(회원가입/로그인), 객관식 문제 풀이(문제은행 목록·채점 조회·세션 저장), 서술형 문제 풀이(세션 시작·AI 채점/꼬리질문·세션 저장).
- 백엔드가 아직 없는 기능(오답노트, 학습 기록, 진척도 등)은 **더미 데이터로 화면만** 유지한다.
