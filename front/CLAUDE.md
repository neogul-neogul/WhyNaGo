...
## 현재 작업 범위 (매우 중요)
- **백엔드가 구현된 기능은 실제 API를 연동한다.** 현재 연동된 기능: 인증(회원가입/로그인), 객관식 문제 풀이(문제은행 목록·채점 조회·세션 저장 — 문항별 소요 시간 측정 포함), 서술형 문제 풀이(세션 시작·AI 채점/꼬리질문·세션 저장), 오답노트(목록·상세 조회·북마크·삭제), 학습 기록(최근 기록 목록·연속/누적 학습일·일자별 학습량 조회 — `/records`, 홈 배너, 헤더 스트릭 배지), 문제집(생성·목록/상세 조회·문제 담기/빼기·삭제 — `/collections`, 문제 풀이 화면의 "문제집에 저장" 모달).
- 백엔드가 아직 없는 기능(진척도, 주간 리포트, 마이페이지 통계 등)은 **더미 데이터로 화면만** 유지한다. 학습 기록 API가 제공하지 않는 값(진입 경로 method, 잔디 등급·점수)도 마찬가지로 프런트에서 만들어내지 않는다 — `lib/records.ts`의 등급 계산은 예외로, 백엔드가 등급을 아예 내려주지 않기로 확정한 부분을 프런트가 대신 계산하는 것이며 새 더미 데이터가 아니다.
- 더미 데이터로만 동작하는 화면은 `src/lib/comingSoon.ts`의 `COMING_SOON_ROUTES`에 등록해 접근을 차단한다. 백엔드 연동이 완료되면 해당 경로를 배열에서 제거한다.
