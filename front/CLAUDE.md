...
## 현재 작업 범위 (매우 중요)
- **백엔드가 구현된 기능은 실제 API를 연동한다.** 현재 연동된 기능: 인증(회원가입/로그인), 객관식 문제 풀이(문제은행 목록·채점 조회·세션 저장 — 문항별 소요 시간 측정 포함), 서술형 문제 풀이(세션 시작·AI 채점/꼬리질문·세션 저장), 오답노트(목록·상세 조회·북마크·삭제), 학습 기록(최근 기록 목록·연속/누적 학습일·일자별 학습량 조회 — `/records`, 홈 배너, 헤더 스트릭 배지), 문제집(생성·목록/상세 조회·문제 담기/빼기·삭제 — `/collections`, 문제 풀이 화면의 "문제집에 저장" 모달), 관리자 백오피스(문제 목록·상세, 대시보드, 회원 관리, 메일 발송 이력 — 배치 실행 이력·수신자별 발송 상세).
