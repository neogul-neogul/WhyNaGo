...
type Sel = { date: string; level: number; count: number; color: string };
            ...
            {weeks.map((week, wi) => (
              <div key={wi} className="flex flex-col gap-[3px]">
                {week.days.map((d, di) => (
                  <div
                    key={di}
                    title={`${d.date} · ${d.count > 0 ? `${d.count}문제 풀이` : "학습 없음"}`}
                    onClick={() => setSel({ date: d.date, level: d.level, count: d.count, color: d.color })}
      ...
      {sel && (
        <div className="flex items-center gap-[18px] rounded-[14px] bg-ink px-[22px] py-[18px] text-white">
          ...
          <span className="text-[14px] font-semibold">
            {sel.date} · {sel.count > 0 ? `학습량 ${sel.level}단계` : "학습 기록 없음"}
          </span>
          <span className="text-[13px] text-placeholder">
            {sel.count > 0 ? `${sel.count}문제 풀이` : "이 날은 학습 기록이 없습니다"}
          </span>
        </div>
      )}
    </>
