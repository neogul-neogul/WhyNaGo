...
문제은행에 노출되는 **본 질문**을 표현하는 엔티티다. 꼬리질문을 다루는 방식은 유형별로 다르다.

- **객관식**: 본 질문과 꼬리질문 모두 이 엔티티로 표현한다. 꼬리질문은 별도 테이블이 아니라, 선택지(`AnswerChoice.relatedQuestionId`)가 가리키는 또 다른 `Question` 이다. → [객관식 꼬리질문 분기·진행 정책](#꼬리질문-분기꼬리-진행-정책) 참고.
- **서술형**: 본 질문만 이 엔티티로 저장한다(`type = ESSAY`, `isRoot = true`). 꼬리질문은 세션마다 AI가 생성하므로 이 엔티티로 저장하지 않는다. → [서술형 꼬리질문 생성 정책](#서술형-꼬리질문-생성-정책) 참고.
...
| isCorrect | 정답 여부 | LLM 채점 통과 여부. 세션 `correctCount` 집계에 사용 (판정 기준은 [보류](#보류-추후-mvp)) |
| solvedAt | 응답 시각 | 개별 문항 채점 시각 (선택) |
...
- 서술형은 본 질문부터 꼬리질문2까지 이어 푸는 하나의 `SolvedSession`(`type = ESSAY`)이다.
  1. **본 질문 조회**: DB의 `Question`(`type = ESSAY`, `isRoot`)을 제시한다.
...
- **서술형 정답 판정 기준**: LLM 채점 결과를 `EssaySolved.isCorrect`(통과/미통과)로 환산하는 방식(점수 임계값 여부 등)은 미정. 세션 `correctCount` 집계 규칙과 함께 추후 확정한다.
