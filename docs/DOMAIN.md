...
| sourceSolvedId | 발생 풀이 ID | FK → SolvedMultipleChoice (최초/최근 오답이 난 풀이) |
| repeatCount | 반복 오답 횟수 | 같은 문제를 다시 틀릴 때마다 증가(상태가 아닌 단순 카운터) |
| bookmarked | 북마크 여부 | 사용자가 별도로 표시 |
| source | 출처 | `PROBLEM_SOLVING`(문제 풀이) \| `SELF_DIAGNOSIS`(자가진단) \| `RETRY`(재풀이) \| `MANUAL`(직접 저장) |
| createdAt | 최초 오답 시각 | |
| lastWrongAt | 최근 오답 시각 | 화면의 `solvedAt` |

> 화면의 `미복습` 필터: 복습 상태가 없으므로, "재풀이(`RETRY`)로 다시 푼 적 없는 오답" 정도로 파생 정의한다(확정 필요 시 별도 논의).
...
- 이미 오답노트에 있는 문제를 다시 틀리면 `repeatCount` 를 증가시킨다.
- **복습 상태(미복습/복습중/해결완료)는 두지 않는다.** 오답노트는 "틀린 문제 목록 + 반복 횟수 + 북마크"만 유지한다.
...
### 문제은행 표시 정책
- 문제은행에는 **본질문(`isRoot`)만** 노출된다. 꼬리질문은 목록에 나타나지 않는다.
...
- **선택지 개수**: 4개 고정(4지선다).
- **오답노트 상태**: 두지 않는다. 틀린 문제 목록 + 반복 횟수 + 북마크만 유지.
...
- **오답노트 `미복습` 필터 정의**: 복습 상태가 없으므로 필터 의미는 추후 확정.
