...
- 객관식과 서술형은 점수를 매기는 단위가 다르다(→ `docs/DOMAIN.md` 꼬리질문 분기·서술형 꼬리질문 생성 정책).
  - **객관식**: 본질문·꼬리질문 모두 문제은행의 실제 독립된 `Question`이므로, 세션에 등장한 **문항 하나하나가 각자 자기 `difficulty`로** 점수를 받는다(하 3점, 중 4점, 상 5점). 예를 들어 본질문(중, 4점)에 꼬리질문(상, 5점)이 이어져 둘 다 맞혔다면 그 세션은 총 9점이다.
  - **서술형**: 꼬리질문은 AI가 그때그때 생성하는 스냅샷이라 실제 `Question`(난이도)이 없으므로, **본질문 하나의 `difficulty`로만** 점수를 매긴다. 점수는 같은 난이도의 객관식 점수의 3배(하 9점, 중 12점, 상 15점) — 3문항(본질문+꼬리질문 2개)을 전부 맞혀야 이 점수를 받는다.
