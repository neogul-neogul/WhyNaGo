...
| --- | --- | --- |
| `categories[].category` | String | 카테고리. |
| `categories[].levelCounts` | Object | 그 카테고리에서 각 숙련도를 받은 **문항 수**. 받은 적 없는 숙련도는 키에서 빠진다. 태그가 없는 문항의 판정도 포함되고, 꼬리질문 턴의 판정은 제외된다. |
...
`levelCounts`는 이력 기반이고 `tags[].level`은 현재값이다. 그래서 같은 태그의 서로 다른 문항을 풀면 `levelCounts`의 합은 늘어나지만 `tags[].level`은 하나만 남는다.

세는 단위가 **문항**인 이유는 한 번의 채점이 문항의 태그 개수만큼 이력 행을 만들기 때문이다. 행을 그대로 세면 태그 3개짜리 문항 하나가 3으로 잡힌다. 같은 이유로 꼬리질문 턴을 제외한다 — 본질문 하나가 최대 3턴을 만들어 같은 문항이 턴 수만큼 다시 세어진다. 단, 같은 문항을 다시 풀어 판정이 바뀌면 두 숙련도 키에 각각 남는다(판정 이력이므로 이전 판정을 지우지 않는다).
