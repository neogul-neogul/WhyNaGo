...
인증된 사용자가 지금까지 푼 문제의 ID를 모아서 반환한다. 문제은행 목록 화면에서 이미 푼 문제를 표시하는 데 쓴다.

정답/오답은 구분하지 않는다. 완료된 풀이 세션만 저장되므로(→ 세션 저장), 여기에 담긴 ID는 "끝까지 풀어 저장된 문제"를 뜻한다.

객관식 풀이 이력(`SolvedMultipleChoice`)과 서술형 풀이 이력(`EssaySolved`)을 함께 모은다. 서술형 꼬리질문은 세션마다 AI가 생성해 참조할 `Question`이 없으므로(→ `docs/DOMAIN.md` 서술형 꼬리질문 생성 정책) 제외된다. 객관식 꼬리질문은 실제 `Question`이므로 포함되며, 문제 목록에 노출되지 않는 ID가 섞여 있을 수 있다.
...
- 성공 시 `200 OK`를 반환한다.
- 정렬·페이징은 없다. 같은 문제를 여러 번 풀어도 ID는 한 번만 담긴다.
...
| **필드** | **타입** | **설명** |
| --- | --- | --- |
| `questionIds` | Array | 푼 문제 ID(`Question.id`) 목록. 중복 없음. 푼 문제가 없으면 빈 배열. |

### **에러**

없음. 푼 문제가 없으면 `questionIds`가 빈 배열이다.
