...
- **서술형 꼬리질문 턴은 별도 출처(`AI_ESSAY_FOLLOWUP`)로 기록하고, 태그별 현재값(`user_tag_mastery`)은 갱신하지 않는다.** 연습 채점과 1일 1면접이 같은 엔드포인트로 모든 턴을 처리하므로 판정 자체는 턴마다 발생한다. 이때 현재값을 덮어쓰면 **가장 깊은 마지막 프로브가 그 태그의 숙련도가 되어**, 본질문을 제대로 답한 사용자가 그 주제를 모르는 것으로 기록된다. 꼬리질문은 본질문보다 깊게 파고들므로 판정이 더 낮게 나오는 것이 정상이고, 그래서 같은 축에 둘 수 없다. 턴 판별은 대화 이력의 완료 턴 수로 하며(`EssayAnswerEvaluator`), 클라이언트가 보고하지 않는다 — 턴을 맡기면 판정 출처를 조작할 수 있다. 이력에 남긴 꼬리질문 판정은 "본질문은 맞혔는데 프로브에서 무너졌다"는 깊이 신호로 쓸 수 있다. 객관식 꼬리질문은 그 자체가 문항이라 `questionId`와 태그가 있으므로 본질문과 똑같이 기록한다.
- 트랙이 달라도 `user_tag_mastery`는 태그당 한 행이다. 같은 태그를 서술형으로도 객관식으로도 풀면 **나중 판정이 이전 판정을 덮어쓴다** — 판정 출처가 아니라 시점이 기준이다.
- 추천의 약점 프로필은 저장된 `RULE_CHOICE` 판정을 읽지 않고 객관식을 매번 다시 계산한다(`MasteryReader.readLatestAiLevelsByQuestion`이 `AI_ESSAY`만 고른다 — 출처를 나눠 뒀으므로 이 쿼리가 꼬리질문 판정도 자동으로 제외한다). 문항 평균 소요시간(`question_stat`)이 매일 재집계되므로, 푼 시점의 기준이 아니라 **현재 기준**으로 판정해야 프로필이 최신 표본을 반영한다. 저장된 값은 `GET /api/mastery`의 사용자 노출용이다.
