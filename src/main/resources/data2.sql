-- ⚠️ [레거시 — 현재 스키마로는 로드할 수 없다]
-- 이 파일은 docs/TAG.md 태그 사전이 생기기 전에 만든 시드다. question_tag가 태그 이름을 직접 갖던
-- 정규화 이전 형식이며, 사용 태그 638종 중 533종이 현재 사전에 없다(4xx, B-Tree, Big-O, CAP 이론 등).
-- tag 테이블 정규화 이후로는 tag_id를 해석할 수 없어 그대로 실행하면 실패한다.
-- 로컬 시드는 data-tag.sql + data.sql + data3.sql이며 data3.sql이 8개 카테고리를 모두 덮는다.
-- 되살리려면 태그 638종을 사전 태그로 재매핑해야 한다(별도 작업).
