...
// v4에 루브릭 기반 채점을 추가한 버전이다.
//
// v4까지는 채점 기준이 모델 머릿속에만 있어 같은 답변이 호출마다 다른 점수를 받을 수 있었고,
// 사용자는 무엇을 빠뜨려 그 점수인지 항목 단위로 알 수 없었다.
// v5는 문항에 저장된 루브릭(question.rubric)을 프롬프트에 내려 항목별 충족 여부를 판정받는다.
//
// 점수는 결국 서버가 충족 항목의 배점 합으로 다시 계산해 덮어쓴다(RubricGradingResolver).
// 그럼에도 프롬프트에 같은 규칙을 적는 것은 feedback·mastery가 그 점수와 모순되지 않게 하기 위해서다.
public class EssayPromptV5 implements EssayPrompt {
    ...
    public String userPrompt(EssayGradingMode mode, EssayGradingTarget target, boolean generateFollowup) {
        ...
        return USER_PROMPT_TEMPLATE.formatted(question, answer, rubricInstruction, followupInstruction);
    }
